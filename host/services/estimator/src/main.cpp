      ...
      while (1)
	{
	  ...
	  while (items--)
		  ...
		  ntohs (pkt->forw.crc))
		{
	      ...
	      if(pkt->forw.hdr.proto != RFBPROTO_BEACONFORWARDER)
		{
			...
			tag_id, &item_mutex))) != NULL)
		{
		  ...
		  printf
		    ("id:%04u reader:%03u proto:%03u button:%03u levels:",
		     tag_id, reader_id & 0xFF, pkt->pkt.proto, tag->button);
		  for (j = 0; j < STRENGTH_LEVELS_COUNT; j++)
