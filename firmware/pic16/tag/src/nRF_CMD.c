...
const unsigned char g_MacroInitialization[] = { 0x01, OP_NOP, 0x02,
		NRF_REG_CONFIG | WRITE_REG, 0x00, // stop nRF
...
// first byte payload size+1, second byte register, 3..n-th byte payload
const unsigned char g_MacroStart[] = { 0x02, NRF_REG_CONFIG | WRITE_REG,
		NRF_CONFIG_BYTE | NRF_CONFIG_PWR_UP, 0x02, NRF_REG_STATUS | WRITE_REG,
		0x70, // reset status
		0x00 };

// first byte payload size+1, second byte register, 3..n-th byte payload
const unsigned char g_MacroStop[] = { 0x02, NRF_REG_CONFIG | WRITE_REG,
		NRF_CONFIG_BYTE, 0x02, NRF_REG_STATUS | WRITE_REG, 0x70, // reset status
		0x00 };
...
unsigned char nRFCMD_RegWrite(unsigned char reg, const unsigned char *buf,
		unsigned char count) {
	unsigned char res;
...
unsigned char nRFCMD_RegRead(unsigned char reg, unsigned char *buf,
		unsigned char count) {
	unsigned char res;
...
void nRFCMD_Stop(void) {
	nRFCMD_Macro(g_MacroStop);
}

void nRFCMD_Execute(void) {
